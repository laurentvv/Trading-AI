...
## [2026-09-01] eval | Audit PROD J+13 du run démo 30j (logs_prod) — 3 anomalies, 1 bug critique trouvé
- Run sain dans l'ensemble : 3 ordres réels (aller-retour 19-20/08 ~flat, achat 28/09 @1458.78), stop GTC présent en permanence, ratchet UP-only observé (1312.90→1314.63 le 01/09 09:32, peak×0.90), GO-gates 1/2/3/5/7 tenus en prod, briefs PASS, council 29/08 généré. Scheduler arrêté ~14:42 par l'utilisateur (PID 12808, celui du 24/08) pour l'audit — redémarrage manuel requis (lock auto-récupéré après 15 min).
- CRITIQUE F1 (bug real P&L) : state SXRVd_EQ realized=-1.4972 € alors que le broker rapporte +1.58 €. Cause : `_fifo_pnl` (t212_executor) itère l'historique T212 tel quel — l'API renvoie les items DU PLUS RÉCENT AU PLUS ANCIEN, donc le SELL 20/08 a été matché contre le lot BUY POSTÉRIEUR du 28/08 (1451.20 vs 1458.80 = -1.4972 exactement). Tout multi-allers-retours inverse la chronologie FIFO.
- ANOMALIE A2 (classic SELL 170/170 sur CRUDP.PA) : PAS un bug de plomberie. Le cycle CRUDP analyse CL=F (« Trading: CRUDP.PA | Analyse: CL=F ») — données WTI propres (0 gelée, targets 47.8/52.2) ; le modèle rejoué offline donne SELL conf 0.62-0.65 = opinion honnête et persistante (WTI a pourtant monté 75→86 $). Le vrai défaut est structurel : un modèle WTI vote pour CRUDP.PA et l'évaluation des perfs n'a pas de colonne ticker (`resolve_previous_predictions` applique le return d'UN ticker à toutes les lignes du jour — constaté : 1 seul return_1d par date pour les 2 tickers). Recommandation post-run : colonne ticker dans model_performance_history + évaluation par couple (ticker, modèle).
- ANOMALIE A3 (données gelées) : le parquet CRUDP_PA (ticker de TRADING) est 82 % gelé (placeholder Volume=0 + Close inchangé ; 2022-2025 = 100 % gelés, 5.562 constant 4 ans). Sans effet sur classic (il analyse CL=F) ni sur le prix live (patch T212), mais piège latent pour tout consommateur futur.
- ANOMALIE A4 (vincent_ganne N/A) : désactivation VOLONTAIRE (enhanced_trading_example.py:711, audit juillet 2026 — VG émettait BUY/STRONG_BUY 147/147 sur SXRV.DE). Rien à corriger.
- ANOMALIE A5 (429) : ~20×429/jour ; `get_t212_positions`/`get_t212_order_history` faisaient un GET direct sans retry → un seul 429 annulait toute la sync (~1 cycle/jour sans sync broker).

## [2026-09-01] fix | Remédiation audit J+13 — 4 correctifs, 271/271 tests, validés live
- F1 `_fifo_pnl` : tri chronologique des items par `fill.filledAt`/`order.createdAt` AVANT matching (`_fill_sort_key`). Validé sur l'historique broker réel : realized -1.4972→+1.5760 (= chiffre broker 1.58, open_cost 289.13). Le state se self-corrigera au premier cycle après redémarrage du scheduler (sync → equity ≈ budget+1.58+unrealized). Test de régression : `test_history_newest_first_is_sorted_chronologically` (items API antéchronologiques).
- A5 : `get_t212_positions`/`get_t212_order_history` routés via `safe_request` (GET idempotents → retry 429 backoff 2s/4s ; la règle « jamais de retry à l'aveugle » ne concerne que les POST d'ordres, inchangés). Mocks de tests compatibles (même transport `_t212_session.request`).
- A3 : masquage des labels sur lignes gelées dans `features.create_features` — Target*/NaN quand (Volume=0 ∧ Close inchangé) ou ligne suivante gelée (le label comparerait au close placeholder, saut d'unfreeze inclus) ; seuil de target calculé sur les rendements réels uniquement. Les lignes RESTENTENT dans le frame (fenêtres MA_200 préservées — un filtre de lignes aurait fait tomber CRUDP sous le garde 50 échantillons → fallback SELL(0.50), testé et écarté). No-op vérifié sur ^NDX/CL=F (0 gelée) et SXRV (3) : aucun changement de comportement courant.
- A2 : PAS corrigé en cours de run (changement de logique de décision = brouille la mesure GO/NO-GO) — documenté en recommandation post-run.
- Tests : 271/271 PASS (suite mockée complète). AGENTS.md §2.2 : 2 invariants ajoutés (tri FIFO chronologique, labels gelés).
- Reste à faire : redémarrer le scheduler (start_scheduler.bat), commit des correctifs.

## [2026-09-02] gen | Migration TimesFM 2.5→3.0 + extension reset logs_prod + runbook migration PROD (run 2)
- Contexte : `timesfm==3.0.1` publié sur PyPI (2026-09-02) avec le package `timesfm3` → abandon du clone vendor patché (`setup_timesfm.py`). API : `TimesFM3Forecaster.from_pretrained("google/timesfm-3.0-pytorch")`, CPU supporté, quantiles natifs (9). Décisions utilisateur : médiane seule (drop-in, seuils inchangés), contexte 2048.
- Aussi : reset du run démo 1 (compromis : 3 ordres/13 j, critère ≥20 round-trips inatteignable, scheduler arrêté depuis le 01/09 14:42) → run 2 de 30 j avec TimesFM 3.0. Poids 3.0 sous licence non-commerciale (flaggué).
- Correctif structurant inclus : `reset_for_fresh_test.py` préserve `logs_prod/` (KEEP_PATHS) alors que TOUS les artefacts runtime PROD y vivent (scheduler lancé depuis logs_prod/) — incident du 24/08 (model_performance.db pré-reset survécu). Extension : détection + `--include-logs-prod`/`--keep-logs-prod`.
- Réalisation : `src/timesfm_model.py` (API timesfm3, contexte `TIMESFM_CONTEXT=2048`, médiane + quantiles en métadonnées, retry `_try_init()` dans predict), `tests/test_timesfm_model.py` (fakes ForecastOutput), `pyproject.toml` timesfm>=3.0.1,<4 sans source vendor, suppression `setup_timesfm.py`/`.gitmodules`/`[tool.uv.sources]`, `main.check_setup()` → find_spec("timesfm3"), fix `scripts/backtest_ensemble_10y.py` (appel predict() invalide, TypeError avalée → TimesFM jamais compté), `reset_for_fresh_test.py` étendu (`--include-logs-prod`/`--keep-logs-prod`, refus --yes sans choix exit 2, `_backup_root()` résolu à l'appel pour tests sandbox), `tests/test_reset_fresh_test.py` (11 tests), `tests/smoke_timesfm3.py`, docs README/AGENTS(invariant §2.2)/QWEN/GEMINI/SYSTEM_SUMMARY/i18n×9, runbook `docs/PLAN_MIGRATION_TIMESFM3_PROD.md`, memory-bank (feature F-03 v3.0 + F-37, changelog, techContext, progress, activeContext).
