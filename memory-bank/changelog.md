# Journal des Corrections (Historique Produit)

> **Note de rôle** — Ce fichier est l'**historique chronologique des corrections produit** (mai 2025 → juin 2026).
> Il a été renommé depuis `progress.md` le 2026-06-30 pour lever une collision de noms avec le nouveau
> système de mémoire déterministe (voir `AGENTS.md §1`).
>
> Rôles distincts dans `memory-bank/` :
> - **`changelog.md`** (ce fichier) — historique produit, append-only, ne pas effacer.
> - **`progress.md`** — tableau de bord du sprint en cours (objectif + jalons).
> - **`log.md`** — journal d'exécution de l'agent (append-only, par action).
> - **`feature_list.json`** / **`contract.md`** — cartographie et critères de validation.
