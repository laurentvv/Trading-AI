...
- `src/council/`: **Weekend Council** — async, multi-persona LLM retrospective (`run_council`). 6 personas on 5 distinct Ollama model families (Gemma 4 12B / GLM-4.6V-Flash / Qwen 3.5 9B / LFM 2.5 / Mistral Nemo 12B), Judge on Qwen3.5-9B-MTP. 4-round protocol + anti-groupthink. Its `VERDICT_TICKER` stance becomes the **11th weighted vote** (9.5%) in the consensus via `get_council_ticker_stance()` in `llm_client.py` (age-decayed over 7 days). Models installed via `setup_council_models.py`. See `docs/ADR-003`.
