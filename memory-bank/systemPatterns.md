...
- **Lean Bridge (Unidirectional Signal Export)**: The `src/lean_bridge.py` reads the production `trading_journal.csv` and converts it into Lean-compatible signal files (CSV + JSON) without ever modifying the production pipeline. The bridge handles mixed-format journals (7-column legacy → 13-column current) and maps European tickers to US proxies (SXRV.DE→QQQ, CRUDP.PA→USO). This ensures backtesting is decoupled from live trading.
- **Lean Validation Gate**: The `src/lean_validator.py` provides a CI/CD-style validation pattern — after any change to the decision engine, a Lean backtest can be run to verify that key metrics (Sharpe Ratio, Max Drawdown, Total Return) do not degrade below configurable thresholds.
