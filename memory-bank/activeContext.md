...
- **Weekend Council as 11th Voice (2026-06-28)**: The weekend council (Level 3) becomes a real weighted vote (9.5%) in the consensus, not just soft context. Each persona runs on a distinct Ollama model family for genuine reasoning diversity. The council is exempt from the adaptive weight loop (`fixed_weight_models`) since its correctness can't be measured against per-cycle market direction; temporal relevance is handled by linear age-decay (full at day 0 → 0 at day 7). A critical code-review fix was needed: the call site initially passed the *analysis* ticker (`^NDX`) instead of the *trading* ticker (`SXRV.DE`), making the vote inert. See `docs/ADR-003`.
