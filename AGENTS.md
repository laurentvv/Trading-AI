...
- **`logs_prod/` is a DEV-only copy (NOT a PROD path)**: on DEV, `logs_prod/` holds a **snapshot copy** of the PROD runtime files (copied PROD → DEV for offline analysis/auditing); it does **not** exist on PROD, and PROD does not write there. Consequences for any agent: (1) **never** suggest deleting or refreshing `logs_prod/data_cache/...` as a PROD remediation — that only wipes the DEV analysis copy and leaves PROD untouched; the real cache lives at `data_cache/...` on the **PROD machine**; (2) all PROD cache-invalidation reminders must target the PROD `data_cache/` path, not `logs_prod/`; (3) when editing or auditing, treat `logs_prod/` as read-only evidence. (This note exists because earlier PRs repeatedly told the user to delete `logs_prod/data_cache/eia/...` after a pull — harmless on DEV but misleading, since it implies PROD writes there.)
...
- **⛔ Kronos (23 July 2026) — DO NOT RE-INTEGRATE.** The foundation model `shiyu-coder/Kronos` (`NeoQuasar/Kronos-base`, AAAI 2026, MIT) was fully integrated as a 12th voting model then **rejected** after a 10-year backtest on the PROD tickers: Kronos-base is pre-trained on 5-min emerging-market K-lines and produces massively bearish signals on our daily European ETFs (0 BUY signal on CRUDP.PA across 1279 bars; +17% vs Buy&Hold +100% on SXRV.DE). Full evidence + re-opening conditions: `memory-bank/log.md` [2026-07-23]. **General lesson**: any new decision model must pass the 10y bench vs Buy&Hold **and** Hold-only before activation.
...
- **EIA crude_imports degenerate cache** (`src/eia_client.py:134-170`) — `get_crude_imports` returned a 1-row payload every cycle. Root cause: a single monthly `period` spans ~2000-3000 rows (origin × grade × destination), and `length=300` + `sort ASC` could never cross a month boundary (ASC even started at 2009-01). **Fix** (PR #86): `sort DESC` (recent months first) + `length=5000` (the API hard cap) → returns the latest ~3 months; payloads with <3 rows are still refused caching (logged WARNING). (NB: `/crude-oil-imports/data` does NOT accept a `facets[process]` or `facets[period]` param — its valid facets are `originId`/`originType`/`destinationId`/`destinationType`/`gradeId`; earlier revisions tried `process` and got HTTP 400 on every cycle.)
...
> **PROD cache invalidation reminder:** after `git pull` on the **PROD machine**, delete the PROD `data_cache/eia/eia_crude_imports.parquet` to force a live re-fetch. (Do **not** touch `logs_prod/data_cache/...` — on DEV that is only an analysis copy of PROD, and on PROD that path does not exist; see the `logs_prod/` note in §5.)
...
**Still open after ADR-002 (partially resolved, see §6.4):** isotonic TensorTrade recalibration (current cap is interim), unifying the two `regime_adjustments` dicts, `return_5d` population. *(The "upstream sentiment-data skew" and the classic-model isotonic calibration were both addressed by the 28 July 2026 over-correction fix — §6.4.)*
...
### 6.4 Resolved follow-ups (28 July 2026 — ADR-002 over-correction: bearish bias)

The 28 July 2026 PROD audit (1 day post-reset, 30 cycles 27-28 July) found that **3 models never emitted BUY**: `classic` (0/30), `llm_visual` (0/30), `sentiment` (0/30, always HOLD). The genuinely down market (-3.1% / -0.6%) justified the SELL votes, but 0 BUY was **structural** — root cause: the June 2026 ADR-002 bullish-bias fix had **over-corrected** in three independent places, creating a symmetric bearish bias. Three fixes:

- **`classic` BUY unreachable — dead-band + isotonic** (`src/classic_model.py:26, 119, 347`) — the ADR-002 HOLD band (`CLASSIC_HOLD_MARGIN=0.08`) required `proba(BUY) ≥ 0.58`, and the ADR-002 isotonic calibration flattened `predict_proba` toward 0.5 on a weak classifier (F1≈0.32, recall≈0.25), so 0.58 was never reached. PROD showed SELL confidence pinned at exactly 0.650 (the cap) every cycle. **Fix**: margin `0.08 → 0.04` (BUY at proba ≥ 0.54) + calibration `isotonic → sigmoid` (Platt preserves more spread on weak classifiers). Cap `0.65` retained. Regression tests `test_buy_signal_reachable_on_bullish_features` / `test_sell_signal_reachable_on_bearish_features` — the gap ADR-002 left: it pinned threshold *symmetry* but not signal *reachability* from the calibrated model.
- **`llm_visual` BUY suppressed — asymmetric prompt** (`src/gemini_gateway.py:187`, `src/llm_client.py:546`) — the ADR-002 "force HOLD on ambiguous" prompt set a low bar for HOLD ("ambiguous, mixed, *or* mostly sideways" = almost any chart) and a very high bar for BUY/SELL ("textbook unmistakable pattern + >0.7 confidence"), at `temperature=0.4`. PROD: 0 BUY / 30 cycles. **Fix**: symmetrized instructions (BUY on "recognizable uptrend (higher lows, breakout)", SELL on "recognizable downtrend", HOLD "ONLY when genuinely directionless", "SAME confidence standard to BUY and SELL") + `temperature 0.4 → 0.6`. Identical wording in both the Gemini path and the Ollama fallback.
- **`sentiment` always ~0 — broken pipeline (3 bugs)** (`src/news_fetcher.py`) — (A) Alpha Vantage rate-limit `break` returned 0 silently (lost headlines from earlier queries); (B) AlphaEar fallback crashed silently (`get_unified_trends()` returns a Markdown *string*, but the code iterated it with `.get("title")` → `AttributeError` swallowed by bare `except` — so the 0.30-weighted second source *never contributed*); (C) the score averaged `ticker_sentiment_score` across **all** tickers in every article (e.g. SPY/MSFT in an oil story), diluting toward ~0.10, below the ±0.15 threshold. **Fix**: log+continue on rate-limit; call `fetch_hot_news(source_id)` (returns `List[Dict]` with `title`); filter `ticker_sentiment` rows to the target ticker.

> **General lesson:** an anti-bias fix (ADR-002) can create a *symmetric* bias if it over-corrects. Always validate not just that the old bias is gone, but that signal **reachability in both directions** survives — via a bull-fixture AND bear-fixture end-to-end test, not just threshold symmetry. The ADR-002 tests pinned `buy_threshold == sell_threshold` but never asserted a calibrated model could *actually emit* BUY — exactly the gap this bug fell through.
...
**Routing convention**: a model string prefixed `gemini:` is routed to the gateway. In the council, the tier is inferred from the id — `"pro"` → paid cascade, otherwise free. The gateway internally runs a **cascade** (head model → safety nets) per tier and the `QuotaTracker` (`src/gemini_quota.py`) pre-flights each call against RPM/RPD caps plus **two billing guards** on the paid tier: (1) a rolling 30-day **cost budget** `GEMINI_PAY_MONTHLY_BUDGET_EUR` (default 8.0 € — set below the ~8.6 € Google AI Studio monthly spend cap so the local guard trips before Google's 429; each call's cost is computed from the real `usage_metadata` token counts × the per-model `PRICE_TABLE_EUR_PER_MTOKEN`), and (2) a daily call-count backstop `GEMINI_PAY_DAILY_CAP` (default 200). When the gateway can't serve a request it returns `None` (never raises) and the caller falls back to local Ollama.
