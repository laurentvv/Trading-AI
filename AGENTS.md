...
- **Stale-at-source price data is refused, download or not (2026-09-10)**: `get_etf_data` (`src/data.py`) raises when the last bar is older than `MAX_STALE_PRICE_CACHE_DAYS` (3 d) even after a SUCCESSFUL download — on 2026-09-08/09 Yahoo kept serving CRUDP.PA ending 2026-09-04 and cycles traded Friday's close for two days. This also protects the weight manager: predictions are dated by the last data bar (see `MODEL_HORIZONS` in `src/adaptive_weight_manager.py`), so a lagging feed mis-dates rows and collapses days of cycles into one via dedup.
- **Brent spot (EIA RBRTE) freshness gate (2026-09-10)**: `get_brent_spot_price` (`src/eia_client.py`) refuses payloads whose latest period is older than `MAX_BRENT_SPOT_AGE_DAYS` (21 d) on EVERY path (HTTP, memory/disk cache, fallback) + a 12 h circuit breaker. The series was stale at source (~100 d at $96.02) and its spread vs live futures computed a FAKE contango (−$5.71 at the top of a +5 % rally) read as bearish by OilBench. The Dated-vs-Futures spread must never be computed from a stale spot leg.
- **Ratchet self-heal adopts standing stops (2026-09-10)**: `_ratchet_stop_order` (`src/t212_executor.py`) reconciles with `/equity/orders` (`_get_active_stop_order`) before placing a "missing" stop — a standing GTC stop already reserves the shares, so a duplicate POST fails `selling-equity-not-owned` (6 ERRORs on 2026-09-03 after the state reset lost the stop ID).
- **A 400 `selling-equity-not-owned` on SELL = possibly stale reads (2026-09-10)**: `_handle_failed_sell` (`src/t212_executor.py`) immediately re-checks the live position; if it is gone, the state is reconciled on the spot and no stop is re-placed. Root cause observed: T212 demo READ endpoints (positions + cash) served the pre-sale snapshot for ~30 min after a confirmed fill (2026-09-08 09:01) while the order endpoint held the truth.
