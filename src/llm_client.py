...
def _query_ollama(payload: dict, max_retries: int = 3) -> dict:
    ...
    for attempt in range(max_retries):
        try:
            response = requests.post(
                OLLAMA_API_URL, json=payload, timeout=120
            )  # Longer timeout for visual models
            ...
            if not all(key in llm_output for key in required_keys):
                logger.warning(
                    f"Attempt {attempt + 1}/{max_retries}: Response from LLM ({model_name}) is malformed: {llm_output}"
                )
            ...
            logger.warning(
                f"Attempt {attempt + 1}/{max_retries} failed for LLM ({model_name}): {e}"
            )
            if attempt < max_retries - 1:
                ...
                return {
                    "signal": "HOLD",
                    "confidence": 0.0,
                    "analysis": f"LLM ({model_name}) failed after {max_retries} attempts: {e}",
                }
        except Exception as e:
    ...
    return {
        "signal": "HOLD",
        "confidence": 0.0,
        "analysis": "Fell through retry loop unexpectedly.",
    }
