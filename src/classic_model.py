...
def _load_cached_model(cache_hash: str):
    ...
    if path.exists():
        try:
            ...
            logger.info(
                f"Modele classique charge depuis le cache (hash={cache_hash[:12]}...)"
            )
            return (
                cached["model"],
                cached["scaler"],
...
def _save_model_cache(cache_hash: str, model, scaler, metrics, feature_importance):
    ...
    try:
        ...
        logger.info(
            f"Modele classique sauvegarde dans le cache (hash={cache_hash[:12]}...)"
        )
    except Exception as e:
...
def train_ensemble_model(X: pd.DataFrame, y: pd.Series) -> tuple:
    ...
    Uses a cache keyed on the data hash — identical data reuses the previous model.
    """
    ...
    for name, model in models.items():
        cv_scores = cross_val_score(
            model, X_train_scaled, y_train, cv=tscv, scoring="f1"
        )
        mean_score = cv_scores.mean()

        logger.info(
            f"{name} - TimeSeries CV Score: {mean_score:.4f} (+/- {cv_scores.std() * 2:.4f})"
        )
...
def get_classic_prediction(
    ...
    latest_features_imputed = latest_features_imputed.replace(
