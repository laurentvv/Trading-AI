...
class AdvancedRiskManager:
    ...
    def should_override_signal(
        ...
        SPECIAL LOGIC: Oil often performs well in high-risk/volatile environments.
        """
        is_oil = any(x in ticker.upper() for x in ["CL=F", "CRUDP", "WTI", "BZ=F"])
        ...
        if signal in ["BUY", "STRONG_BUY"]:
            if (
                ...
                return (
                    True,
                    f"Extreme risk ({risk_metrics.risk_level.name}){trend_msg}{oil_msg} still requires minimal confidence ({min_buy_confidence:.2f})",
                )
        ...
        if is_holding and original_signal in ["SELL", "STRONG_SELL"]:
            ...
            if entry_price_index and price_data is not None:
                ...
                if confidence < sell_threshold:
                    return (
                        "HOLD",
                        f"EXIT INERTIA: Protecting position. Signal conviction ({confidence:.2f}) < threshold ({sell_threshold:.2f}). Index Perf: {index_perf:+.2%}",
                    )
