...
from kronos_model import KronosForecaster
...
# Adjusted weights to include the new Kronos model.
W_CLASSIC = 0.30
W_LLM_TEXT = 0.20
W_LLM_VISUAL = 0.20
W_SENTIMENT = 0.05
W_KRONOS = 0.25  # New model

# Placeholder for future macro weight
W_MACRO = (
    0.0  # Currently not directly used in final decision, but fetched for context/LMM
)
...
def get_hybrid_decision(
    ...
    text_llm_score = signal_map.get(
        text_llm_decision.get("signal", "HOLD").upper(), 0
    ) * text_llm_decision.get("confidence", 0.0)

    visual_llm_score = signal_map.get(
        visual_llm_decision.get("signal", "HOLD").upper(), 0
    ) * visual_llm_decision.get("confidence", 0.0)

    sentiment_score = signal_map.get(
        sentiment_decision.get("signal", "HOLD").upper(), 0
    ) * sentiment_decision.get("confidence", 0.0)

    kronos_score = signal_map.get(
        kronos_decision.get("signal", "HOLD").upper(), 0
    ) * kronos_decision.get("confidence", 0.0)

    final_score = (
        (classic_score * W_CLASSIC)
        + (text_llm_score * W_LLM_TEXT)
        + (visual_llm_score * W_LLM_VISUAL)
        + (sentiment_score * W_SENTIMENT)
        + (kronos_score * W_KRONOS)
    )
...
def plot_analysis(data, backtest_data):
    ...
    ax1.scatter(
        backtest_data.index[buy_points],
        backtest_data["Close"][buy_points],
        color="green",
        marker="^",
        s=100,
        label="Buy",
    ...
    ax1.scatter(
        backtest_data.index[sell_points],
        backtest_data["Close"][sell_points],
        color="red",
        marker="v",
        s=100,
        label="Sell",
    ...
    ax2 = axes[1]
    ax2.plot(
        backtest_data.index, backtest_data["Cumulative_Strategy"], label="Strategy"
    )
    ax2.plot(
        backtest_data.index,
        backtest_data["Cumulative_Returns"],
...
def run_walk_forward_backtest(
    ...
    with tqdm(total=num_iterations, desc="Walk-Forward Backtest") as pbar:
        start_index = 0
        while start_index + train_period_days + test_period_days <= len(
            data_with_features
        ):
            ...
            fold_signals = []
            feature_cols = [
                col for col in X_train.columns if col in scaler.feature_names_in_
            ]
            ...
            for i in range(len(test_data)):
                ...
                classic_pred, classic_conf = get_classic_prediction(
                    classic_model, scaler, current_features_subset
                )
                ...
                sentiment_sim = {
                    "signal": sim_signal,
                    "confidence": sim_conf,
                }  # Simple simulation for sentiment

                kronos_sim = {"signal": sim_signal, "confidence": sim_conf} # Simple simulation for Kronos
                final_decision, _ = get_hybrid_decision(
                    classic_pred,
                    classic_conf,
                    text_llm_sim,
                    visual_llm_sim,
...
def main():
    ...
    # --- Initialize Kronos Forecaster ---
    logger.info("Initializing Kronos Forecaster...")
    ...
    backtest_data, performance = run_walk_forward_backtest(
        data_with_features, 252, 63, macro_context={}, ticker=TICKER
    )  # Pass ticker
    ...
    if len(y) == 0:
        ...
        logger.warning(
            "Target variable has only one class in the training set. Cannot train the final model properly. Skipping final model training and prediction."
        )
        ...
        if X.isna().any().any():
            logger.warning(
                "Feature matrix X contains NaNs before scaling. Filling with 0."
            )
            X = X.fillna(0)
        ...
        if pd.isna(X_scaled).any():
            logger.error(
                "Scaled feature matrix X_scaled contains NaNs. Cannot train the model."
            )
            final_classic_model = None
            final_scaler = None
            classic_pred = 0
    ...
    feature_cols = [
        ...
        if col in getattr(final_scaler, "feature_names_in_", X.columns)
    ]
    ...
    if latest_features_subset.isna().any().any():
        logger.warning(
            "Latest feature subset for prediction contains NaNs. Filling with 0."
        )
    ...
    chart_generated = generate_chart_image(
        data_with_features, CHART_OUTPUT_PATH, title=f"{TICKER} - 6 Month Chart"
    )

    # Get predictions from the four models
    if final_classic_model is not None:
        classic_pred, classic_conf = get_classic_prediction(
            final_classic_model, final_scaler, latest_features_subset
        )
    ...
    text_llm_decision = get_llm_decision(
        latest_data_without_macro
    )  # Use data without macro for LLMs to keep context consistent
    visual_llm_decision = (
        get_visual_llm_decision(CHART_OUTPUT_PATH)
        if chart_generated
        else {
            "signal": "HOLD",
            "confidence": 0.0,
    ...
    try:
        ...
        logger.info(
            f"Successfully fetched news. Overall sentiment score: {sentiment_score:.2f}"
        )
    except (
    ...
    # Get prediction from Kronos model
    logger.info("Getting prediction from Kronos model...")
    # The history for Kronos should include the latest day. We use the last 512 data points as context.
    kronos_history = data_with_features_final.tail(512).copy()
    kronos_decision = kronos_forecaster.predict(kronos_history)
    logger.info(
        f"Kronos Decision: {kronos_decision['signal']} (Confidence: {kronos_decision['confidence']:.2f})"
    )

    # Get final hybrid decision
    final_decision, final_score = get_hybrid_decision(
        classic_pred,
        classic_conf,
        text_llm_decision,
        visual_llm_decision,
    ...
    insert_model_signal(
        ...
        TICKER,
        "classic",
        "BUY" if classic_pred == 1 else "SELL",
        classic_conf,
    )
    insert_model_signal(
        analysis_date.strftime("%Y-%m-%d"),
    ...
    insert_model_signal(
        ...
        TICKER,
        "kronos",
        kronos_decision.get("signal", "HOLD"),
        kronos_decision.get("confidence", 0.0),
    )
    ...
    table.add_row(
        ...
        Text(
            translate_signal(classic_signal_en),
            style=get_signal_style(classic_signal_en),
        ),
        f"{classic_conf:.2%}",
    )
    ...
    table.add_row(
        ...
        Text(
            translate_signal(text_llm_signal_en),
            style=get_signal_style(text_llm_signal_en),
        ),
    ...
    table.add_row(
        ...
        Text(
            translate_signal(visual_llm_signal_en),
            style=get_signal_style(visual_llm_signal_en),
        ),
    ...
    table.add_row(
        ...
        Text(
            translate_signal(sentiment_signal_en),
            style=get_signal_style(sentiment_signal_en),
        ),
        f"{sentiment_decision.get('confidence', 0.0):.2%}",
    )

    kronos_signal_en = kronos_decision.get("signal", "HOLD")
    table.add_row(
        "Time-Series (Kronos)",
        Text(
            translate_signal(kronos_signal_en), style=get_signal_style(kronos_signal_en)
        ),
        f"{kronos_decision.get('confidence', 0.0):.2%}",
    )
    ...
    decision_text = Text(
        translate_signal(final_decision),
        justify="center",
        style=get_signal_style(final_decision),
    )
    score_text = Text(f"Score: {final_score:.4f}", justify="center")
    reliability_text = Text(
        f"Fiabilité: {reliability_fr}", justify="center", style=reliability_style
    )

    output_text = Text.assemble(decision_text, "\n", score_text, "\n", reliability_text)
    ...
    if latest_state:
        ...
        portfolio_info_table.add_row(
            "Valeur Totale Portefeuille", "Inconnue (Aucune donnée)"
        )
        portfolio_info_table.add_row(
            "Valeur Benchmark (B&H)", "Inconnue (Aucune donnée)"
        )
    ...
    if latest_trans:
        ...
        portfolio_info_table.add_row(
            "Dernière Transaction", f"{trans_type} {trans_qty:.2f} @ ${trans_price:.2f}"
        )
    ...
    # --- END NEW ---
    console.print(
        Panel(output_text, title="[bold]Décision Finale[/bold]", border_style="bold")
    )
    console.print("")
