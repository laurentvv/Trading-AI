...
from llm_client import get_llm_decision
...
W_CLASSIC = 0.5  # Poids du modèle classique
W_LLM = 0.5      # Poids du modèle LLM

def get_hybrid_decision(classic_pred: int, classic_conf: float, llm_decision: dict) -> tuple[str, float]:
    """
    Combine les décisions du modèle classique et du LLM pour une décision finale.
    """
    signal_map = {"BUY": 1, "HOLD": 0, "SELL": -1}
    classic_score = 1 if classic_pred == 1 else -1
    llm_score = signal_map.get(llm_decision.get('signal', 'HOLD').upper(), 0)
    llm_conf = llm_decision.get('confidence', 0.0)
    final_score = (classic_score * classic_conf * W_CLASSIC) + (llm_score * llm_conf * W_LLM)
...
def run_walk_forward_backtest(data_with_features: pd.DataFrame, train_period_days: int, test_period_days: int):
    """
    Exécute un backtest en utilisant la validation walk-forward pour éviter le lookahead bias.
    """
    all_signals = []
        ...
        while start_index + train_period_days + test_period_days <= len(data_with_features):
            ...
            for i in range(len(test_data)):
                ...
                if classic_pred == 1 and classic_conf > 0.7:
                    llm_sim_signal = "BUY"; llm_sim_conf = 0.8
                elif classic_pred == 0 and classic_conf > 0.7:
                    llm_sim_signal = "SELL"; llm_sim_conf = 0.8
                else:
                    llm_sim_signal = "HOLD"; llm_sim_conf = 0.5

                llm_decision_sim = {"signal": llm_sim_signal, "confidence": llm_sim_conf}
                final_decision, _ = get_hybrid_decision(classic_pred, classic_conf, llm_decision_sim)
...
def main():
    ...
    """
    TICKER = 'QQQ'  # Remplacé 'FR0011871110.PA' qui ne semble plus disponible
    ...
    backtest_data, performance = run_walk_forward_backtest(
        data_with_features,
        train_period_days=252,  # 1 an de données pour l'entraînement
        test_period_days=63    # 3 mois de données pour le test
    )
    ...
    llm_decision = get_llm_decision(latest_data)

    final_decision, final_score = get_hybrid_decision(classic_pred, classic_conf, llm_decision)

    logger.info("\n=== DÉCISION FINALE HYBRIDE ===")
    logger.info(f"Prédiction Classique : {'BUY' if classic_pred == 1 else 'SELL/HOLD'} (Confiance: {classic_conf:.2f})")
    logger.info(f"Décision LLM         : {llm_decision.get('signal')} (Confiance: {llm_decision.get('confidence', 0.0):.2f})")
    logger.info(f"Analyse LLM          : {llm_decision.get('analysis')}")
    logger.info("-" * 30)
