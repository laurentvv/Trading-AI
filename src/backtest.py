...
def _calculate_performance_metrics(df: pd.DataFrame) -> dict:
    ...
    strategy_sharpe = (
        (strategy_annual - risk_free_rate) / strategy_vol if strategy_vol > 0 else 0
    )
    benchmark_sharpe = (
        (benchmark_annual - risk_free_rate) / benchmark_vol if benchmark_vol > 0 else 0
    )
    ...
    win_rate = (
        len(winning_trades) / len(strategy_returns[strategy_returns != 0])
    ...
    for date in df.index:
        ...
        if date in trade_dates:
            ...
            current_position = (
                0
                if date == df.index[0]
                else df.loc[df.index[df.index.get_loc(date) - 1], "Position"]
            )
        ...
        benchmark_value = (
            df.loc[date, "Cumulative_Returns"] * initial_cash
        )  # Assuming initial investment of initial_cash in benchmark
