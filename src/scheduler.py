...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', filename='scheduler.log', filemode='a')
...
class IntelligentScheduler:
    def __init__(self, project_root):
        ...
        self.current_phase = self.config.get('current_phase', 'Phase 1: Configuration & Test')
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for key, value in config.items():
                    cursor.execute("REPLACE INTO scheduler_config (key, value) VALUES (?, ?)", (key, json.dumps(value)))
                conn.commit()
    ...
    def run_daily_analysis(self):
        ...
        try:
            ...
            logger.error(f"[ERROR] An unexpected error occurred during daily analysis: {e}")
    ...
    def start(self):
        ...
        schedule.every().day.at(self.config.get('schedule', {}).get('daily_analysis', '18:00')).do(self.run_daily_analysis)
    ...
    def generate_weekly_report(self):
        ...
        try:
            ...
            logger.info("Weekly report generation logic not implemented in this version.")
    ...
    def update_phase_progress(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("REPLACE INTO phase_progress (phase, progress) VALUES (?, ?)",
                               (self.current_phase, 0.5)) # Using a float value
                conn.commit()
