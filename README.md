...
- **Weekend Council models** (optional, but required for the council's reasoning diversity): the council runs each persona on a *different* model family (Gemma / GLM / Qwen / Gemini). Install them all at once with `uv run python setup_council_models.py`.
